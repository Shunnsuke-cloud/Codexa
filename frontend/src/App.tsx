...
import { getDashboardSummary, getWeeklyReport, postGithubPoll, getGithubHistory } from './services/reportsService';
...
function Shell() {
  ...
  const [history, setHistory] = React.useState<Array<{startedAt:string; finishedAt?:string; status:string; syncedCount?:number; message?:string}> | null>(null);
        ...
        <main className="grid flex-1 gap-6 lg:grid-cols-[1.5fr,1fr]">
          ...
          <aside className="grid gap-4">
              <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
                ...
                {!user ? (
                  <div className="mt-4 text-sm text-slate-400">同期機能はログイン後に利用できます。<Link to="/login" className="text-cyan-300 underline ml-2">Sign In</Link></div>
                ) : (
                  <>
                    <div className="mt-4 flex gap-2">
                      <button onClick={async () => {
                        ...
                        try {
                          ...
                          setHistory(h);
                        } catch(e) { /* ignore */ } finally { setSyncing(false); }
